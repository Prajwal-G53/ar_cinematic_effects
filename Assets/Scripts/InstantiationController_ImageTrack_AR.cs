...
public class InstantiationController_ImageTrack_AR : MonoBehaviour
{
    ...
    InputController_AR inputController_AR_script;
    public TMPro.TextMeshProUGUI message_inst;

    public bool planes_started=false;
    ...
    private void Update()
    {
        if (TrackedImages.added_track && isAREnabled&& planes_started==false) { AskUserToPlace(); planes_started = true; }
    ...
    private void ARInstantiationRoutine(Vector2 screenPos)
    {
        ...
        if (TrackedImages.added_track) message_inst.text = "ARInstantiationRoutine() Called";
    ...
    {
        message_inst.text = "Inst Called";
        if (TrackedImages.added_track)
        {
            message_inst.text = "Instantiated....";
            Vector3 new_pos = new Vector3(TrackedImages.pose_img.position.x, pose.position.y, TrackedImages.pose_img.position.z);
            poseForMultipleObjects.position = new_pos;// pose.position;
            ...
            if (prefabInScene == null)
            {
                ...
                ModelParent.transform.SetPositionAndRotation(new_pos, pose.rotation);
                prefabClone = Instantiate(objectPrefab, new_pos, pose.rotation);
    ...
    {
        if(TrackedImages.added_track)   message_inst.text = "Tap to place Called";
        if (!Application.isEditor)
        {
